...
function makeAjaxCall(url, methodType, meth, id) {
  var promiseObj = new Promise(function(resolve, reject) {
    ...
    xhr.send(JSON.stringify({ jsonrpc: "2.0", id: id, method: meth }));
...
async function fetchData(meth, id) {
  var rtnData = await makeAjaxCall(URL, "POST", meth, id).then(
    processResponse,
    errorHandler
  );
  return rtnData;
...
function verifySign1(resData, instance) {
  ...
  //console.log("Uint val length" + toSolUint256("1").length);
...
function verifySignedBy(sgn, addr, fingerprint, instance) {
  return instance.signedBys.call(fingerprint, sgn, addr);
}

function verifySignedByBoth(sgn0, addr0, sgn1, addr1, fingerprint, instance) {
  return instance.signedByBoths.call(fingerprint, sgn0, sgn1, addr0, addr1);
}

function verifySignedByOne(sgn0, addr0, addr1, fingerprint, instance) {
  return instance.signedByOnes.call(fingerprint, sgn0, addr0, addr1);
}
